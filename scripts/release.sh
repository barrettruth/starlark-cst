...
Examples:
  just release 0.1.0
  just release 0.1.1 --dry-run
  just release 0.1.1 --tag
EOF
}
...
Pushed $tag.
The release workflow publishes to crates.io from the tag.
EOF
  exit 0
fi
...
# The workflow cannot publish without this, and finding that out after the PR
# has merged means a tag pointing at a release that never happened.
if command -v gh >/dev/null 2>&1; then
  if ! gh secret list --json name --jq '.[].name' 2>/dev/null | grep -qx 'CARGO_REGISTRY_TOKEN'; then
    die "repository secret CARGO_REGISTRY_TOKEN is not configured"
  fi
else
  die "gh is required to verify repository secrets"
fi
...
run git add Cargo.toml Cargo.lock flake.nix
...
The crate and Nix package metadata need bumping from $current_version to $version.

## Solution

Run the release preparation script for $version, including repo CI and \`cargo publish --dry-run --locked --allow-dirty\`, and route the release commit through branch protection.
EOF
)"
